<?php Defined("BASE_PATH") or die("Dilarang Mengakses File Secara Langsung"); ?>

<!-- menu beranda -->
<li class="menu-beranda"><a href="<?= BASE_URL ?>"><i class="fa fa-link"></i> <span>Beranda</span></a></li>
<!-- menu data proyek -->
<li class="menu-proyek"><a href="<?= BASE_URL."proyek"; ?>"><i class="fa fa-link"></i> <span>Data Proyek</span></a></li>
<!-- menu data bank -->
<li class="menu-bank"><a href="<?= BASE_URL."bank"; ?>"><i class="fa fa-link"></i> <span>Data Bank</span></a></li>
<!-- menu data pengajuan kas kecil -->
<li class=""><a href="<?= BASE_URL."pengajuan-kas-kecil"; ?>"><i class="fa fa-link"></i> <span>Data Pengajuan Kas Kecil</span></a></li>
<!-- menu data operasional proyek -->
<li class=""><a href="<?= BASE_URL."operasional-proyek"; ?>"><i class="fa fa-link"></i> <span>Data Operasional Proyek</span></a></li>
<!-- menu data operasional -->
<li class=""><a href="<?= BASE_URL."operasional"; ?>"><i class="fa fa-link"></i> <span>Data Operasional</span></a></li>
<!-- menu data kas kecil -->
<li class=""><a href="<?= BASE_URL."kas-kecil"; ?>"><i class="fa fa-link"></i> <span>Data Kas Kecil</span></a></li>
<!-- menu data sub kas kecil -->
<li class=""><a href="<?= BASE_URL."sub-kas-kecil"; ?>"><i class="fa fa-link"></i> <span>Data Sub Kas Kecil</span></a></li>
<!-- menu data user -->
<li class=""><a href="<?= BASE_URL."user"; ?>"><i class="fa fa-link"></i> <span>Data User</span></a></li>