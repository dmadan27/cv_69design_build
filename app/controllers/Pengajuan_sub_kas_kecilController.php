...
class Pengajuan_sub_kas_kecil extends Controller 
{
	...
	public function get_list_detail($id) {
		if($_SERVER['REQUEST_METHOD'] == "POST" && ($_SESSION['sess_level'] === 'KAS BESAR' 
			|| $_SESSION['sess_level'] === 'OWNER' || $_SESSION['sess_level'] === 'KAS KECIL')) {
			$id = strtoupper($id);
			// config datatable
			$config_dataTable = array(
				'tabel' => 'v_detail_pengajuan_sub_kas_kecil',
				'kolomOrder' => array(null, 'nama', 'jenis', 'satuan', 'qty', 'harga', 'subtotal'),
				'kolomCari' => array('nama', 'jenis', 'satuan', 'qty', 'harga', 'subtotal'),
				'orderBy' => array('id' => 'asc'),
				'kondisi' => 'WHERE id_pengajuan = "'.$id.'"',
			);
			...
			foreach($dataDetail as $row){
				...
				$dataRow[] = $row['satuan'];
				$dataRow[] = $row['qty'];
				$dataRow[] = $this->helper->cetakRupiah($row['harga']);
				$dataRow[] = $this->helper->cetakRupiah($row['subtotal']);
